...
<?php
   ...
   //---> Rubrique valide ?
   $rubrique_id = getRubriqueId("background");
 ...
 //  include "include/pagination.php";       //---> Utiliser le module pagination	
   ...
   {
	 background_supprimer($_POST['supprimer']);
   } //Fsi
   ...
   if (isset($_POST['id']))
   {
     ...
     if (isset($_POST['visible']))
	   background_visible($_POST['visible'],$_POST['id']);
	 else
	   background_visible(NULL,$_POST['id']);  // Tous à faux
   } //Fsi
  ...
  **********************************************************************************************************/
  ...
  $p = new CAdminPagination("background","", 10, "filename");
...
<!--
  ...
  function ajouter()
  {
    popup('background_add.php?session=<?php echo $session?>&lang=<?php echo $lang?>',525,350);
  } //Fin ajouter
-->
</script>
      ...
      <tr>
        <td><table width="100%" align="center" border="0" cellpadding="2" cellspacing="1" class="text">
          <tr class="tcat">
            ...
            <td><a href="<?php echo $p->makelink('title_en', $p->courent)?>" class="text"> FileName </a></td>           
            <?php 
			     if($mod=='Y')
	             { 
	          ?>
            <td width="40"><a href="<?php echo $p->makelink('background_visible', $p->courent)?>" class="text"> Visible </a></td>
            <?php
			     } //Fsi
	             if($delete=='Y')
	             { 
          ...
          <?php
	          ...
	          while($row=@mysql_fetch_array($res))
	          {
				...
				$background_visible = ($row['background_visible']=='Y')? "CHECKED" : "";
          ...
          <tr bgcolor="<?php echo $color?>" id         = "<?php echo $i?>"
            ...
            <td height="25" align="left" class="inactif"><?php echo $p->courent*$p->page+$i?>
              <input type="hidden" name="id[]" value="<?php echo $row['background_id']?>" /></td>
            <td><?php 
				if($mod=='Y') 
		        {
		      ?>
              <a href="javascript:popup('background_add.php?session=<?php echo $session?>&lang=<?php echo $lang?>&background_id=<?php echo $row['background_id']?>',525,350);" class="menutext"> <img src="../../background/<?php echo $filename; ?>" alt="Photo actualite" width="75" height="75" border="0" align="absmiddle"> </a>
              <?php
				} else 
				{
				?>
				<img src="../../background/<?php echo $title_en; ?>" alt="Photo actualite" width="75" height="75" border="0" align="absmiddle">
				<?php				  
				} //Fsi
              ?></td>
            ...
            <?php 
		        ...
		        {   
	          ?>
            <td align="center"><input type="Checkbox" name="visible[]" value="<?php echo $row['background_id']?>" style="color:#666666;" <?php echo $background_visible?> /></td>
            <?php
			    } //Fsi
	            ...
	            { 
	          ?>
            <td align="center"><input type="Checkbox" name="supprimer[]" id="supprimer<?php echo $i?>" value="<?php echo $row['background_id']?>"  onclick="javascript: restore_row(this.parentNode.parentNode, '<?php echo $color?>');" /></td>
            <?php
			    } //Fsi
			  ?>
          </tr>
