...
session_start();
	...
	if($_POST['requester_id']){
		$requester_id  = $_POST['requester_id'];
 		$requesterOld  = get_requester($requester_id);
		
		//UPDATE  THE REQUEST  
		if(update_request($_POST)){
			$requesterUpdated = get_requester($requester_id);
			$RequestertDiff = array_diff($requesterOld, $requesterUpdated);
		}
		else{
			$requesterUpdated = $requesterOld;
			$RequestertDiff = '';
		}
 
		extract($requesterUpdated);
	  	$NbreSite  = $NbreSite;
					...
					$template='	<tr>
							  <td width="50%" align="left" valign="top"  style=" padding:0px 0 0px 50px; font-family:Arial,
							  Helvetica, sans-serif; font-size:14px;">
							  <strong style="color:#228cbe; font-size:18px;">'.$lang['contact_info'].' <br></strong> 
							  
							  	'.Display_info($requesterUpdated, "contact_info");
								
					if(count($RequestertDiff))	{
						 $template .='<br><br><strong style="color:red; font-size:18px;">'.$lang['contact_info_old'].' <br></strong> 
						 			<span >'.Display_info($RequestertDiff, "contact_info").'</span>';
					}
					 ...
					 $template .='<p><br></p></td>
							  ...
							  <strong style="color:#228cbe; font-size:18px;">'.$lang['booking_detail'] .'<br></strong>
								'.Display_info($requesterUpdated, "booking_detail");
					...
					if($RequestertDiff)	{
							 $template .='
							 <br><br><strong style="color:red; font-size:18px;">'.$lang['booking_detail_old'].' <br></strong> 
							 <span  >'.Display_info($RequestertDiff, "booking_detail").'</span>';
					}								
							...
							<tr>
							  <td colspan="3" style="font-size:14px; padding:10px 0 0px 0px;   "><table  border="0" cellspacing="0" cellpadding="0">
								  <tr>
									...
									<strong style="color:#228cbe; font-size:18px;ont-weight:bold; ">'.$lang['site_info'].'<br>  
									</strong></td>
								  </tr>
								  <tr>
									<td width="230"  style="padding-left:50px;"  align="left">
									<img src="http://tamaani.ca/images/bg_three_1.png" ></td>
									<td width="230"  style="padding-left:50px;"  align="left">
									<img src="http://tamaani.ca/images/bg_three_2.png" ></td>
									<td width="230"  style="padding-left:50px;"   >
									<img src="http://tamaani.ca/images/bg_three_3.png" ></td>
								  </tr>
								  <tr>
			...
			$siteChoosedOld = get_requester_site($requester_id);
				
			//REMOVE ALL THE OLD SITES
			 remove_requester_site($requester_id);	
			...
			for ($j = 1; $j <= $NbreSite; $j++) {
				...
				/*if(!$changed)
					if (!in_array(array('site_id' =>$requester_site['site_id']), get_requester_siteID($requester_id)))	
						$changed = true;*/
 

				
				$template .='<td width="315"  align="left" style="padding-left:50px; padding-top:10px; font-family:Arial, 
				Helvetica, sans-serif; font-size:14px; ">
				...
				//if($changed) {
						$template .='</tr><tr><td width="315"  align="left" style="padding-left:50px; padding-top:10px; font-family:Arial, 
							Helvetica, sans-serif; font-size:14px; ">
							<strong style="color:red; font-size:18px;ont-weight:bold; ">'.$lang['site_sammary_old'].' </strong><br>';
				
						foreach($siteChoosedOld as $siteCh ){
							 $template .=$siteCh['SiteName'].'	<br>';
						}
		...
		$Subject           = 'Video Conference Modification # '.$Reference ;
