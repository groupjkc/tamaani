...
if($_POST['CompanyName'])
{
	...
	if(is_int($requester_id))
	{
		...
		if (send_confirmation_email($header_requester, $Subject, $Email, utf8_decode($ContactName), utf8_decode($template), $type, $footer))
		{
			$result = 1;
		}
		else
		...
		if ($result == 1)
		{
			if (send_confirmation_email($header_team, $Subject, TO_EMAIL_conferencing, TO_NAME_conferencing, utf8_decode($template), $type, $footer))
			{
				$result = 2;
			}
			else
		...
		{	
			if (send_confirmation_email($header_room_man, $Subject, $site['RREmail'], $site['RRContact'], $template, $type, $footer))
			{
				$result++;
			}
			else
