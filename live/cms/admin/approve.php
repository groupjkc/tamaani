 ...
 include "include/operation_message.php";
   ...
   { //---> il s'agit d'une modification
	 ...
	 if ($row==FALSE)
	   die("Error");	     
		...
		$TimeEnd		    = stripslashes($row['TimeEnd']);		
		 ...
		 $DateVCStart        =  $DateVC.' '.$TimeStart;	

		  
		$DateVCEnd          =  $DateVC.' '.$TimeEnd;	
		 
	  	$DateVCStartEchange= date( "Ymd His", strtotime( "$DateVCStart + 4 hours" ) );
		$DateVCEndEchange= date( "Ymd His", strtotime( "$DateVCEnd + 4 hours" ) );
	...
	if (isset($_POST['CompanyName'])){
		...
		$InfoPartText= $lang['contact_info'].'\n\n'.$lang['CompanyName'].': '.$CompanyName.'\n'.$lang['PO'].': '.$PO.'\n'.$lang['ContactName'].': '.$ContactName.'\n'.$lang['Address'].': '.$Address.'\n'.$lang['Town'].': '.$Town.'\n'.$lang['Province'].': '.$Province.'\n'.$lang['Zip'].': '.$Zip.'\n'.$lang['Email'].': '.$Email.'\n'.$lang['WorkPhone'].': '.$WorkPhone.'\n\n\n'.$lang['booking_detail'].'\n'.$lang['DateVC'].': '.$DateVC.'\n'.$lang['TimeStart'].': '.$TimeStart.'\n'.$lang['TimeEnd'].     ': '.$TimeEnd.'\n'.$lang['NbreSite'].    ': '.$NbreSite.'\n\n'.$lang['NunavikSite'].': '.$NunavikSite.'\n'.$lang['SpecialNeeds'].': '.$SpecialNeeds.'\n'.$lang['FirstTimeUser'].': '.$FirstTimeUser.'\n\n';
			...
			while($rowSite = @mysql_fetch_array($resSites))
			{
				...
				$InfoPartText .='SITE   '.$i.' \n'.$lang['SiteName'].':  '.$SiteName.'\n
										...
										'.$lang['PHONE'].': '.$RRPhone.'	\n
										'.$lang['Tec'].': \n\n
										'.$lang['NAME'].': '.$TecContact.'	\n
										'.$lang['Email'].': '.$TecEmail.'	\n
										'.$lang['PHONE'].': '.$TecPhone.'	\n'.$lang['TechnologyUsed'].':  '.$TechnologyUsed.'\n';		
										...
										if($TechnologyUsed=='IP') {
											...
											$InfoPartText .='ISDN number: '.$ISDNnumber.'	\n';
										}	
				 ...
				 $InfoPart .= $lang['Equipment'].': '.$Equipment.'	\n
							'.$lang['Model'].': '.$Model.'	\n\n'	;
			...
			$link_update       = "http://www.tamaani.ca/?Reference=".$Reference."&lang=".$Language;
			...
			$header_teamText       = $lang['Approved_team_body'].'\n\n'; 	 							
			
			$header_requester  = $lang['Approved_body'].'
								<br><br> '.$lang['Mod_text'].' <a href="'.$link_update.'&Code='.$code_requester.'#FormMod">
			...
			$typeText             = $lang['Approved_subject'].' # '.$Reference.'\n\n' ;
			...
			foreach ($requester_site as $site) {
				...
				$ATTENDEES .= "ATTENDEE;CN=".$site['RRContact'].";ROLE=REQ-PARTICIPANT;RSVP=TRUE:mailto:".$site['RREmail'].'
';
 
			 	send_confirmation_email($header_room_man, $Subject, $site['RREmail'], $site['RRContact'], $template, $type, $footer);
			}	
$DateVC    =str_replace('-', '', $DateVC);
...
END:VTIMEZONE
BEGIN:VEVENT
ORGANIZER;CN="'.TO_NAME_conferencing.'":mailto:'.TO_EMAIL_conferencing.'"
CLASS:PUBLIC
UID:'.$Reference.'
...
$DateVCStartEchange   = str_replace(' ', 'T', $DateVCStartEchange);
$DateVCEndEchange     = str_replace(' ', 'T', $DateVCEndEchange);
...
CLASS:PUBLIC
UID:'.$Reference.'
DTSTART:'.$DateVCStartEchange.'
DTEND:'.$DateVCEndEchange.'
ATTENDEE;CN=Tamaani-CAL-VC;CUTYPE=RESOURCE;ROLE=NON-PARTICIPANT:invalid:nom
 ail
CONTACT:Tammani 
SUMMARY:'.$lang['Request'].' # '.$Reference.'
UID:'.$code_teamn.'
DESCRIPTION:'.$typeText.'The following video conferencing form has been approved.\nPlease add it to your schedule\n\n\n'.$InfoPartText.'
LOCATION:Tamaani
CATEGORIES:VC Tamaani
STATUS:CONFIRMED
URL:http://TAMAANI.CA
...
END:VCALENDAR';
			...
			send_mail($Subject, TO_EMAIL_cal, TO_NAME_cal, $invitation);
			send_confirmation_email($header_requester, $Subject, $Email, $ContactName, $template, $type, $footer, $CalFileAttachement);
