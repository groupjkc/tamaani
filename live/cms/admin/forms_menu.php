...
<?php 
   //---> Rubrique valide ?
   $rubrique_id = getRubriqueId("forms");
 ...
 //  include "include/pagination.php";       //---> Utiliser le module pagination	
   ...
   {
	 forms_supprimer($_POST['supprimer']);
   } //Fsi
   ...
   if (isset($_POST['id']))
   {
     ...
     if (isset($_POST['visible']))
	   forms_visible($_POST['visible'],$_POST['id']);
	 else
	   forms_visible(NULL,$_POST['id']);  // Tous à faux
   } //Fsi
  ...
  **********************************************************************************************************/
  ...
  $p = new CAdminPagination("forms","", 10, "title_en");
...
<!--
  ...
  function ajouter()
  {
    popup('forms_add.php?session=<?php echo $session?>&lang=<?php echo $lang?>',525,350);
  } //Fin ajouter
-->
</script>
        ...
        <td><table width="100%" align="center" border="0" cellpadding="2" cellspacing="1" class="text">
          <tr class="tcat">
            ...
            <?php 
	             ...
	             { 
	          ?>
            <td width="40"><a href="<?php echo $p->makelink('forms_visible', $p->courent)?>" class="text"> Visible </a></td>
            <?php
			     } //Fsi
	             if($delete=='Y')
	             { 
          ...
          <?php
	          ...
	          while($row=@mysql_fetch_array($res))
	          {
				...
				$forms_visible = ($row['forms_visible']=='Y')? "CHECKED" : "";
          ...
          <tr bgcolor="<?php echo $color?>" id         = "<?php echo $i?>"
            ...
            <td height="25" align="left" class="inactif"><?php echo $p->courent*$p->page+$i?>
              <input type="hidden" name="id[]" value="<?php echo $row['forms_id']?>" /></td>
            <td><?php 
				if($mod=='Y') 
		        {
		      ?>
              <a href="javascript:popup('forms_add.php?session=<?php echo $session?>&lang=<?php echo $lang?>&forms_id=<?php echo $row['forms_id']?>',525,350);" class="menutext"> <?php echo $title_en?> </a>
              <?php
				} else 
				{
				  echo $title_en;
            ...
            <?php 
		        ...
		        {   
	          ?>
            <td align="center"><input type="Checkbox" name="visible[]" value="<?php echo $row['forms_id']?>" style="color:#666666;" <?php echo $forms_visible?> /></td>
            <?php
			    } //Fsi
	            ...
	            { 
	          ?>
            <td align="center"><input type="Checkbox" name="supprimer[]" id="supprimer<?php echo $i?>" value="<?php echo $row['forms_id']?>"  onclick="javascript: restore_row(this.parentNode.parentNode, '<?php echo $color?>');" /></td>
            <?php
			    } //Fsi
			  ?>
          </tr>
